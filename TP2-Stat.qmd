...
Cette section s'appuie sur la [partie 3 du tutoriel](https://cmaugis.github.io/TutorielR-V2025/Part3-StatR.html){target="_blank"}. 
Avant de commencer ce TP, vous devez avoir fait la partie `ggplot2()` du TP1 et/ou avoir lu la présentation de `ggplot2` du [tutoriel 4](https://cmaugis.github.io/TutorielR-V2025/Part4-Ravance.html#visualisation-avec-ggplot2){target="_blank"}. 
...
FreqQual= data.frame(Eff = EffQual, 
                     Freq = EffQual/length(Data$Qualite), FreqCumul=cumsum(EffQual)/length(Data$Qualite))
...
df <- data.frame(Qualite = levels(Qualite_rec), 
                 value = table(Qualite_rec),
...
g1 <- ggplot(Data) + 
  ...
  ggtitle("Effectifs")+xlab("Qualite")
g2 <- ggplot(Data) + 
  geom_bar(aes(x = Qualite_rec, y = ..prop.., group = 1)) +     ggtitle("Frequences")+xlab("Qualite")
g3 <- ggplot(df, aes(x = Qualite, y = valuecumul)) + 
    geom_bar(stat = "identity") +
    ggtitle("Fréquences cumulées")
g4 <- ggplot(df, aes(x = "", y = freq, fill = Qualite)) + 
  geom_bar(width = 1, stat = "identity") +
  coord_polar("y", start = 0)
...
La variance $s_x^2 = \displaystyle\frac{1}{n}\underset{i=1}{\stackrel{n}{\sum}} (x_i - \bar{x})^2$, la variance corrigée $var(\underline{x})=\displaystyle\frac{1}{n-1}\underset{i=1}{\stackrel{n}{\sum}} (x_i - \bar{x})^2$ et l'écart-type corrigé $\sqrt{var(\underline{x})}$. 
...
$\underset{i=1}{\stackrel{n}{\sum}} \mathbb{1}_{x_i\geq m} \geq \frac{n}{2} \textrm{ et } \underset{i=1}{\stackrel{n}{\sum}} \mathbb{1}_{x_i\leq m} \geq \frac{n}{2}$.
```{r}
median(Data$Alcool)
...
+ le troisième quartile $q_{0.75}$ est une valeur qui sépare les 25% des valeurs supérieures de l'échantillon du reste.
On retrouve ces valeurs dans la représentation par boxplot (voir plus bas). 
...
$t\in\mathbb{R}\mapsto F_n(t) = \displaystyle\frac{1}{n} \underset{i=1}{\stackrel{n}{\sum}} \mathbb{1}_{x_i\leq t}$
...
##### Boîte à moustaches / boxplot {#sectboxplot}
